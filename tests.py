...
def baseTests():
    tests = ["baseTests()"]
    tests.append((calc("123"), "123"))
    tests.append((calc("1+1"), "2"))
    tests.append((calc("1+1 +1+ 1 + 1"), "5"))
    tests.append((calc("1- 0- 1"), "0"))
    tests.append((calc("1- -- 1"), "0"))
    tests.append((calc("1---1"), "0"))
    tests.append((calc("1--1"), "2"))
    printTestsAndNo(tests)
...
def conventionConversionTests():
    ...
    tests.append((calc("123456789", 2), "123 456 789"))
    tests.append((calc("123456789", 2), "123 456 789"))
    tests.append((calc("23456789123456789123456789", 2), "23 456 789 123 456 789 123 456 789"))
    tests.append((calc("123456789", 3), "123,456,789"))
    tests.append((calc("1234567", 3), "1,234,567"))
    tests.append((calc("234567", 3), "234,567"))
    printTestsAndNo(tests)
...
def testAdd():
    ...
    tests.append((calc("1+1"), "2"))
    tests.append((calc("1+1 +1+ 1 + 1"), "5"))
    tests.append((calc("123 + 123"), "246"))
    tests.append((calc("0 + 123"), "123"))
    tests.append((calc("0 + 0"), "0"))
    tests.append((calc("123 + 0"), "123"))
    tests.append((calc("23456543434   +   1092471728468"), "1115928271902"))
    if handleFloat:
        tests.append((calc("4.123   +   4.125"), "8.248"))
    printTestsAndNo(tests)
...
def testSubstract():
    ...
    tests.append((calc("1--1"), "2"))
    tests.append((calc("-1-1 -1- 1 - 1"), "-5"))
    tests.append((calc("1-1 -1"), "-1"))
    tests.append((calc("1-1"), "0"))
    tests.append((calc("123 - 123"), "0"))
    tests.append((calc("12 - 123"), "-111"))
    tests.append((calc("12 - 0"), "12"))
    tests.append((calc("0 -12- 0"), "-12"))
    tests.append((calc("0- 12 -0"), "-12"))
    tests.append((calc("458791296437812469   -   761254768172467"), "458030041669640002"))
    if handleFloat:
        tests.append((calc("12.345   -   2.9456"), "9.3994"))
    printTestsAndNo(tests)

def testDivide():
    tests = ["testDivide()"]
    tests.append((calc("12 / 0"), ERR_prefix+ERR_Div0))
    tests.append((calc("0 / 12"), "0"))
    tests.append((calc("0 / 0"), ERR_prefix+ERR_Div0))
    tests.append((calc("12 / -1"), "-12"))
    tests.append((calc("12 / 2"), "6"))
    tests.append((calc("12 / -2"), "-6"))
    tests.append((calc("12 / -2 / -1"), "6"))
    tests.append((calc("465718976546578798875125   //   345678765456789"), "1347259430"))
    if handleFloat:
        tests.append((calc("465718976546578798875125   /   345678765456789"), "1347259430.09882458663526241160768182640359"))
        tests.append((calc("123456/123"), "1003.7073170731707317073170731707317"))
    printTestsAndNo(tests)

def testMultiply():
    tests = ["testMultiply()"]
    tests.append((calc("12 * 0"), "0"))
    tests.append((calc("0 * 12"), "0"))
    tests.append((calc("0 * 0"), "0"))
    tests.append((calc("12 * -1"), "-12"))
    tests.append((calc("12 * 2"), "24"))
    tests.append((calc("12 * -2"), "-24"))
    tests.append((calc("12 * -2 * -1"), "24"))
    tests.append((calc("12 * - 2 *-1"), "24"))
    tests.append((calc("91284817318 * 23784"), "2171118095091312"))
    tests.append((calc("91284817318 * 78213 * 1273865"), "9094962243100062596910"))
    if handleFloat:
        tests.append((calc("3456.34   *   3456.345"), "11946303.4773"))
    printTestsAndNo(tests)
...
def testAdvanced():
    ...
    tests.append((calc("-4!"), "-24"))
    tests.append((calc("(-4)!"), ERR_prefix+ERR_invalid))
    tests.append((calc("4!*3"), "72"))
    tests.append((calc("2*4!"), "48"))
    tests.append((calc("(2*2)!"), "24"))
    tests.append((calc("(4)!"), "24"))
    tests.append((calc("()!"), ERR_prefix+ERR_OpPar))
    tests.append((calc("1+(2*2)!-1"), "24"))
    tests.append((calc("1+(2*2)!*1"), "25"))
    tests.append((calc("5(2*2)!"), "120"))
    tests.append((calc("5V4"), "10"))
    tests.append((calc("5V"), ERR_prefix+ERR_invalid))
    tests.append((calc("5*V4"), "10"))
    tests.append((calc("5V(2*2)!"), "10"))
    tests.append((calc("5V((2*2)!)"), "24.4948974278317809819728407470589"))
    tests.append((calc("2^3!"), "64"))
    tests.append((calc("2**3!"), "64"))
    tests.append((calc("V(2**3!)"), "8"))
    # tests.append((calc("V2**3!"), "8")) # TODO: make this work
    tests.append((calc("2^!"), ERR_prefix+ERR_invalid))
    tests.append((calc("2^^3!"), ERR_prefix+ERR_BadOp))
    tests.append((calc("2***3"), ERR_prefix+ERR_BadOp))
    tests.append((calc("2**/3"), ERR_prefix+ERR_BadOp))
...
def testParenthesis():
    ...
    tests.append((calc("(2+3)+2*3"), "11"))
    tests.append((calc("-2*(1+1)+1-6"), "-9"))
    tests.append((calc("-(2*(1+1)+1)-6"), "-11"))
    tests.append((calc("-((2/2)*(2)/(1+1)+1)-6"), "-8"))
    tests.append((calc("-(2*1/(1+1)+1)-6"), "-8"))
    tests.append((calc("-(((2*((1+1))+1)))-6"), "-11"))
    tests.append((calc("-(-(-(2*((1+1))+1)))-6"), "-11"))
    tests.append((calc("---+-+-(-(-(2*(--(1+1))+1)))-6"), "-11"))
    tests.append((calc("-(1+1)*2+(4)+(5-10/2)-((1+1)/2-1)"), "0"))
    tests.append((calc("-(1+1)*2+(4)+(5-10/2)-((1+1)/2-1) + (1 + 1)"), "2"))
    try:
        tests.append((calc("1-()"), ERR_prefix+ERR_OpPar))
    except Exception as err:
        tests.append((str(err), ERR_prefix+ERR_OpPar))
    try:
        tests.append((calc("---+-+-(-(-(2*(--(1+1))+1)-))-6"), ERR_prefix+ERR_BadOp))
        #                                                   ^-- here is the error, ")-)" doesn't mean anything
    except Exception as err:
        tests.append((str(err), ERR_prefix+ERR_BadOp))
    try:
        tests.append((calc("---+-+-((-(-(2*(--(1+1))+1)))-6"), ERR_prefix+ERR_OpPar))
...
def do_heavyTestingCalc():
    ...
    tests.append((calc(" 1 + 1 / 1287469297356 * 10000000000000"), "8.7671755128735202121"))

    tests.append((calc(""), "0.07759787177658268984599095433463"))
    tests.append((calc("234567987654323456789876543456789876543*98765456789876543456787654.12214124214214-234567987654323456789876543456789876543*98765456789876543456787654.12214124214214"), "0"))
    tests.append((calc("6543456789.8765432345678/1276545687.7654321234567876543-6543456789.8765432345678/1276545687.7654321234567876543"), "0"))
    tests.append((calc("(234567987654323456789876543456789876543*98765456789876543456787654.12214124214214 + 6543456789.8765432345678/1276545687.7654321234567876543) - (234567987654323456789876543456789876543*98765456789876543456787654.12214124214214 + 6543456789.8765432345678/1276545687.7654321234567876543)"), "0"))
    # TODO: check this test as it seems wrong
    tests.append((calc("987654567-87654345678*87456789/8543212345678.876543234567+76543234567.987654323456787654323456789876543234567-654234567/876542345678765432*76543234567898765432+876543456789-43223456789987.8765434567/654321+(6543234567.6543212345-7654345678.876543234)"), "895765861781.343842729539448887925182778484243234567"))
    ...
    tests.append((calc("12356789987654345678765432345678765432345678+876543234567887654321234567-87654321234567887654323456789876543*5432345+765432345676543234567/7654-765432345678987654323456789", 2), "11 880 621 473 967 347 098 216 795 575 707 789 524 487 812,9998693493598118630781290828325"))
    tests.append((calc("5678765432345678+567887654321234567-567887654323456789876543*5432345+32345676543234567/7654-003767654345678543345678902345678987654323456789", 2), "-3 767 654 345 678 546 430 640 561 870 864 292 210 475 512 187,0001306506401881369218709171675"))
    tests.append((calc("12356789987654345678765432345678765432345678+876543234567887654321234567-87654321234567887654323456789876543*5432345+765432345676543234567/7654-765432345678987654323456789", 3), "11,880,621,473,967,347,098,216,795,575,707,789,524,487,812.9998693493598118630781290828325"))
