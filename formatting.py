...
def convertToScientific(string):
    if len(string) < 33:
        return string
    floatIdx = string.index(".") if "." in string else 0
    if floatIdx > 0:
        string = string[:floatIdx]+string[floatIdx + 1:]
    return string[0]+"."+string[1:33]+"e"+str((floatIdx if floatIdx else len(string)) - 1)
...
def convertResult(result: str, convention: int):
    ...
    if convention == 2:
        ...
        return convertToScientific(result)
